module.exports = {
        ...
        return {
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{ text: '🏠 Номера и цены', callback_data: 'rooms' }],
                    [{ text: '🏖 Развлечения', callback_data: 'entertainment' }],
                    [{ text: '🍽️ Удобства', callback_data: 'facilities' }],
    ...
    getBookingKeyboard() {
        return {
            reply_markup: {
                inline_keyboard: [
                    [{ text: '✅ Да, был(а)', callback_data: 'booking_yes' }],
                    [{ text: '❌ Нет, впервые', callback_data: 'booking_no' }],
                    [{ text: '🔙 Главное меню', callback_data: 'back_to_menu' }]
                ]
            }
        };
    ...
    getRoomsKeyboard(roomsData) {
        ...
        if (roomsData && Array.isArray(roomsData)) {
            roomsData.forEach((room, index) => {
                ...
                const roomName = room?.Название || room?.название || `Номер ${index + 1}`;
    ...
    getAdminKeyboard() {
        return {
            reply_markup: {
                inline_keyboard: [
                    [
                        { text: '📋 Ожидающие вопросы', callback_data: 'pending_questions' },
                        { text: '📊 Статистика', callback_data: 'admin_stats' }
                    ],
                    [
                        { text: '⚙️ Настройки', callback_data: 'admin_settings' },
                        { text: '📝 База знаний', callback_data: 'knowledge_base' }
                    ],
                    [
                        { text: '🔙 Главное меню', callback_data: 'back_to_menu' }
                    ]
    ...
    getAnswerManagementKeyboard(userId) {
        return {
            reply_markup: {
                inline_keyboard: [
                    [
                        { text: '✅ Ответить', callback_data: `answer_question_${userId}` },
                        { text: '❌ Удалить', callback_data: `delete_question_${userId}` }
                    ],
                    [
                        { text: '🔙 К вопросам', callback_data: 'pending_questions' }
                    ]
                ]
            }
        };
    ...
    getPendingQuestionsKeyboard(questions) {
        ...
        if (questions && questions.size > 0) {
            for (const [userId, questionData] of questions) {
                ...
                const questionPreview = question.length > 30 
        ...
        if (!questions || questions.size === 0) {
            keyboard.push([{ text: '📭 Нет ожидающих вопросов', callback_data: 'no_questions' }]);
        }
    ...
    getConfirmationKeyboard(actionType, data = '') {
        return {
            reply_markup: {
                inline_keyboard: [
                    [
                        { text: '✅ Да', callback_data: `confirm_${actionType}_${data}` },
                        { text: '❌ Нет', callback_data: `cancel_${actionType}_${data}` }
                    ]
                ]
            }
        };
